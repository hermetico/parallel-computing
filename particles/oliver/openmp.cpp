...
{   
	...
	omp_lock_t* bin_locks = (omp_lock_t*) malloc(g * sizeof(omp_lock_t));
	//initialise locks
	for (int l = 0; l < g; ++l)
		omp_init_lock(bin_locks + l);
  double simulation_time = read_timer();
	
	#pragma omp parallel private(dmin, j, p, q) default(shared)
	{
      ...
      #pragma omp for reduction (+:navg) reduction(+:davg)
			...
			// move particles
			// reduction allowed as these values are just added up which can be done locally
      //# pragma omp for reduction (+:navg) reduction(+:davg) 
      #pragma omp for nowait
			...
			if(find_option( argc, argv, "-no" ) == -1)
			{
				...
				// dmin local, so every thread must execute
				#pragma omp critical
				if (dmin < absmin) 
					absmin = dmin;
